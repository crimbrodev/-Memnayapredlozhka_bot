...
# DATABASE_URL не нужен для SQLite
...
def is_user_banned(user_id: int, channel_id: str = None) -> bool:
    ...
    cur = conn.cursor()
    if channel_id:
        cur.execute("SELECT user_id FROM banned_users WHERE user_id = ? AND channel_id = ?", (user_id, channel_id))
    else:
...
def unban_user(user_id: int, channel_id: str = None):
    ...
    cur = conn.cursor()
    if channel_id:
        cur.execute("DELETE FROM banned_users WHERE user_id = ? AND channel_id = ?", (user_id, channel_id))
    else:
...
def get_banned_users(channel_id: str = None):
    ...
    cur = conn.cursor()
    if channel_id:
        cur.execute("SELECT user_id, username, banned_at, banned_by FROM banned_users WHERE channel_id = ? ORDER BY banned_at DESC", (channel_id,))
    else:
...
def get_pending_posts(channel_id: str):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT id, user_id, username, photo_file_id, caption, created_at FROM pending_posts WHERE channel_id = ? ORDER BY created_at ASC", (channel_id,))
    posts = cur.fetchall()
    cur.close()
    conn.close()
    return posts
...
def remove_pending_post(post_id: int):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("DELETE FROM pending_posts WHERE id = ?", (post_id,))
    conn.commit()
    cur.close()
    conn.close()
...
def get_channel_admins(channel_id: str):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT user_id FROM channel_admins WHERE channel_id = ?", (channel_id,))
...
def is_channel_admin(user_id: int, channel_id: str = None) -> bool:
    ...
    cur = conn.cursor()
    if channel_id:
        cur.execute("SELECT user_id FROM channel_admins WHERE user_id = ? AND channel_id = ?", (user_id, channel_id))
    else:
        cur.execute("SELECT user_id FROM channel_admins WHERE user_id = ?", (user_id,))
    result = cur.fetchone()
    cur.close()
    conn.close()
    return result is not None
...
def get_user_channels(user_id: int):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT channel_id FROM channel_admins WHERE user_id = ?", (user_id,))
    channels = [row[0] for row in cur.fetchall()]
    cur.close()
    conn.close()
    return channels
...
def get_channel_settings(channel_id: str):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT post_interval_minutes, max_posts_per_day, require_caption, allowed_media_types, spam_filter_enabled, last_post_time, allow_global_posts FROM channel_settings WHERE channel_id = ?", (channel_id,))
    result = cur.fetchone()
    cur.close()
    conn.close()
    if result:
...
def get_scheduled_posts(channel_id: str = None):
    ...
    cur = conn.cursor()
    if channel_id:
        cur.execute("SELECT id, channel_id, user_id, username, photo_file_id, caption, scheduled_time FROM scheduled_posts WHERE channel_id = ? ORDER BY scheduled_time ASC", (channel_id,))
    else:
        now = datetime.now()
        cur.execute("SELECT id, channel_id, user_id, username, photo_file_id, caption, scheduled_time FROM scheduled_posts WHERE scheduled_time <= ? ORDER BY scheduled_time ASC", (now,))
    posts = cur.fetchall()
    cur.close()
    conn.close()
    return posts
...
def update_post_reactions(channel_id: str, message_id: int, reactions: int):
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE published_posts SET reactions = ? WHERE channel_id = ? AND message_id = ?",
...
def get_user_balance(user_id: int):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT balance, total_earned FROM user_coins WHERE user_id = ?", (user_id,))
    result = cur.fetchone()
    cur.close()
    conn.close()
...
def update_streak(user_id: int, username: str):
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT current_streak, longest_streak, last_post_date FROM user_streaks WHERE user_id = ?", (user_id,))
    result = cur.fetchone()
    today = date.today()
    ...
    if not result:
        ...
        if last_date == today:
            ...
            cur.execute("UPDATE user_streaks SET current_streak = ?, longest_streak = ?, last_post_date = ? WHERE user_id = ?", (current, longest, today, user_id))
            if current == 7:
                add_coins(user_id, username, 50, "🔥 Стрик 7 дней")
            elif current == 30:
                add_coins(user_id, username, 300, "🔥 Стрик 30 дней")
        else:
            cur.execute("UPDATE user_streaks SET current_streak = 1, last_post_date = ? WHERE user_id = ?", (today, user_id))
    conn.commit()
    cur.close()
    conn.close()
...
def get_streak(user_id: int):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT current_streak, longest_streak FROM user_streaks WHERE user_id = ?", (user_id,))
    result = cur.fetchone()
    cur.close()
    conn.close()
    return result if result else (0, 0)
...
def check_daily_quests(user_id: int, username: str):
    ...
    cur.execute("SELECT quest_type, completed FROM daily_quests WHERE user_id = ? AND quest_date = ?", (user_id, today))
    ...
    if posts_today >= 1 and not quests.get('post_1'):
        cur.execute("UPDATE daily_quests SET completed = 1, completed_at = datetime('now') WHERE user_id = ? AND quest_date = ? AND quest_type = 'post_1'", (user_id, today))
        add_coins(user_id, username, 10, "✅ Задание: 1 мем")
    
    if posts_today >= 3 and not quests.get('post_3'):
        cur.execute("UPDATE daily_quests SET completed = 1, completed_at = datetime('now') WHERE user_id = ? AND quest_date = ? AND quest_type = 'post_3'", (user_id, today))
        add_coins(user_id, username, 30, "✅ Задание: 3 мема")
    
    if posts_today >= 5 and not quests.get('post_5'):
        cur.execute("UPDATE daily_quests SET completed = 1, completed_at = datetime('now') WHERE user_id = ? AND quest_date = ? AND quest_type = 'post_5'", (user_id, today))
    ...
    if streak_result and streak_result[0] >= 3 and not quests.get('streak_3'):
        cur.execute("UPDATE daily_quests SET completed = 1, completed_at = datetime('now') WHERE user_id = ? AND quest_date = ? AND quest_type = 'streak_3'", (user_id, today))
...
def get_daily_quests(user_id: int):
    ...
    cur = conn.cursor()
    today = date.today()
    cur.execute("SELECT quest_type, completed, reward FROM daily_quests WHERE user_id = ? AND quest_date = ?", (user_id, today))
    quests = cur.fetchall()
    cur.close()
    conn.close()
    return quests
...
def has_active_item(user_id: int, item_type: str):
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT id FROM shop_purchases WHERE user_id = ? AND item_type = ? AND used = 0 AND (expires_at IS NULL OR expires_at > ?)", (user_id, item_type, datetime.now()))
    result = cur.fetchone()
    cur.close()
    conn.close()
    return result is not None
...
def use_shop_item(user_id: int, item_type: str, channel_id: str = None):
    ...
    cur = conn.cursor()
    if channel_id:
        cur.execute("UPDATE shop_purchases SET used = 1 WHERE ctid = (SELECT ctid FROM shop_purchases WHERE user_id = ? AND item_type = ? AND channel_id = ? AND used = 0 LIMIT 1)", (user_id, item_type, channel_id))
    else:
        cur.execute("UPDATE shop_purchases SET used = 1 WHERE ctid = (SELECT ctid FROM shop_purchases WHERE user_id = ? AND item_type = ? AND used = 0 LIMIT 1)", (user_id, item_type))
    conn.commit()
    cur.close()
    conn.close()
...
def has_active_subscription(user_id: int):
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT subscription_type FROM user_subscriptions WHERE user_id = ? AND expires_at > ?", (user_id, datetime.now()))
    result = cur.fetchone()
    cur.close()
    conn.close()
...
def has_channel_protection(user_id: int, channel_id: str):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT id FROM channel_protections WHERE user_id = ? AND channel_id = ? AND used = 0", (user_id, channel_id))
    result = cur.fetchone()
    cur.close()
    conn.close()
    return result is not None
...
def use_channel_protection(user_id: int, channel_id: str):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("UPDATE channel_protections SET used = 1 WHERE ctid = (SELECT ctid FROM channel_protections WHERE user_id = ? AND channel_id = ? AND used = 0 LIMIT 1)", (user_id, channel_id))
    conn.commit()
    cur.close()
    conn.close()
...
def get_audit_log(channel_id: str, limit: int = 50):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT action, user_id, admin_id, details, created_at FROM audit_log WHERE channel_id = ? ORDER BY created_at DESC LIMIT ?", (channel_id, limit))
    logs = cur.fetchall()
    cur.close()
    conn.close()
    return logs
...
def is_channel_creator(user_id: int, channel_id: str) -> bool:
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT added_by FROM channels WHERE channel_id = ?", (channel_id,))
    result = cur.fetchone()
    cur.close()
    conn.close()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0].startswith('ref_'):
        ...
        # Находим реферера
        cur.execute("SELECT user_id FROM referral_codes WHERE code = ?", (ref_code,))
        referrer = cur.fetchone()
        
        if referrer and referrer[0] != user_id:
            referrer_id = referrer[0]
            
            # Проверяем, не использовал ли уже этот пользователь реферальную систему
            ...
            if not existing:
                ...
                cur.execute("SELECT username FROM user_coins WHERE user_id = ?", (referrer_id,))
                ref_username = cur.fetchone()
                ref_username = ref_username[0] if ref_username else f"user_{referrer_id}"
                
                add_coins(referrer_id, ref_username, 100, f"🎁 Реферал: @{username}")
                
                # Уведомляем реферера
                try:
                    await context.bot.send_message(referrer_id, f"🎉 По вашей ссылке зарегистрировался @{username}!\n💰 +100 мемкоинов")
                except:
                    pass
                
                await update.message.reply_text("🎁 Вы зарегистрировались по реферальной ссылке!\n💰 Ваш друг получил +100 монет")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data and context.user_data.get('awaiting_pay_amount'):
        try:
            ...
            if recipient.startswith('@'):
                ...
                conn = get_db_connection()
                cur = conn.cursor()
                cur.execute("SELECT user_id FROM user_coins WHERE username = ?", (recipient_username,))
                result = cur.fetchone()
                cur.close()
                conn.close()
                if result:
                ...
                conn = get_db_connection()
                cur = conn.cursor()
                cur.execute("SELECT username FROM user_coins WHERE user_id = ?", (recipient_id,))
                result = cur.fetchone()
                cur.close()
                conn.close()
                if result:
    ...
    if not context.user_data or not context.user_data.get('waiting_for_channel'):
        logger.info(f"Skipping channel search: user_data={bool(context.user_data)}, waiting={context.user_data.get('waiting_for_channel') if context.user_data else None}")
        return
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "selectch":
        ...
        if refund_type == "sub":
            ...
            conn = get_db_connection()
            cur = conn.cursor()
            cur.execute("SELECT subscription_type, created_at FROM user_subscriptions WHERE user_id = ? AND expires_at > datetime('now')", (user_id,))
        ...
        if action == "app":
            try:
                ...
                if settings['interval'] > 0:
                    # Находим последнее время: либо last_post, либо последний запланированный пост
                    last_time = settings['last_post'] or datetime.now()
                    
                    # Проверяем запланированные посты
                    scheduled = get_scheduled_posts(channel_id)
                    if scheduled:
                        last_scheduled_time = scheduled[-1][6]  # scheduled_time последнего поста
                        if last_scheduled_time > last_time:
                            last_time = last_scheduled_time
                    
                    next_post_time = last_time + timedelta(minutes=settings['interval'])
...
async def balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("SELECT amount, reason, created_at FROM coin_transactions WHERE user_id = ? ORDER BY created_at DESC LIMIT 10", (user_id,))
        transactions = cur.fetchall()
        
        # Проверяем подписку
        cur.execute("SELECT subscription_type, expires_at, created_at FROM user_subscriptions WHERE user_id = ? AND expires_at > datetime('now')", (user_id,))
        sub_info = cur.fetchone()
        cur.close()
        conn.close()
...
async def process_update(update: Update, context: ContextTypes.DEFAULT_TYPE, message_id: int, reactions: int):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("UPDATE published_posts SET reactions = ? WHERE message_id = ?", (reactions, message_id))
    rows = cur.rowcount
    conn.commit()
    cur.close()
    conn.close()
...
async def lootbox(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if available == 0:
        ...
        conn.close()
        return
    cur.execute("SELECT id FROM lootboxes WHERE user_id = ? AND opened = 0 LIMIT 1", (user_id,))
    ...
    cur.execute("SELECT completed FROM daily_quests WHERE user_id = ? AND quest_date = ? AND quest_type = 'open_lootbox'", (user_id, today))
    quest_result = cur.fetchone()
    if quest_result and not quest_result[0]:
        cur.execute("UPDATE daily_quests SET completed = 1, completed_at = datetime('now') WHERE user_id = ? AND quest_date = ? AND quest_type = 'open_lootbox'", (user_id, today))
...
async def referral(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT code FROM referral_codes WHERE user_id = ?", (user_id,))
    result = cur.fetchone()
    if not result:
    ...
    try:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS banned_users (
                ...
                channel_id VARCHAR(255),
                username VARCHAR(255),
                banned_at TIMESTAMP DEFAULT datetime('now'),
                banned_by BIGINT,
...
async def update_reactions(context: ContextTypes.DEFAULT_TYPE):
    ...
    cur.execute(
        ...
        "WHERE published_at > datetime('now') - /* 7 days - используйте Python timedelta */ "
...
async def check_expired_subscriptions(context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT user_id, subscription_type FROM user_subscriptions WHERE expires_at <= ? AND expires_at > ?", (datetime.now(), datetime.now() - timedelta(hours=1)))
